...
export const IntegratedDashboard: React.FC<IntegratedDashboardProps> = ({
  ...
  useEffect(() => {
    if (auditReport) {
      setIntegrationStatus({
        overallHealth:
          auditReport.summary.successRate >= 95
            ? "healthy"
            : auditReport.summary.successRate >= 80
              ? "warning"
              : "error",
  ...
  const _handleEmergencyMessage = async (messageId: string) => {
    try {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
        <div className="text-center">
          <div
            className={`w-3 h-3 rounded-full mx-auto mb-1 ${
              integrationStatus?.dataSyncHealth?.convexToWeaviate?.status === "healthy"
                ? "bg-green-500"
                : integrationStatus?.dataSyncHealth?.convexToWeaviate?.status === "warning"
                  ? "bg-yellow-500"
                  : "bg-red-500"
            }`}
          />
        ...
        <div className="text-center">
          <div
            className={`w-3 h-3 rounded-full mx-auto mb-1 ${
              integrationStatus?.dataSyncHealth?.communicationsToMap?.status === "healthy"
                ? "bg-green-500"
                : integrationStatus?.dataSyncHealth?.communicationsToMap?.status === "warning"
                  ? "bg-yellow-500"
                  : "bg-red-500"
            }`}
          />
        ...
        <div className="text-center">
          <div
            className={`w-3 h-3 rounded-full mx-auto mb-1 ${
              integrationStatus?.dataSyncHealth?.clientPortalToSystems?.status === "healthy"
                ? "bg-green-500"
                : integrationStatus?.dataSyncHealth?.clientPortalToSystems?.status === "warning"
                  ? "bg-yellow-500"
                  : "bg-red-500"
            }`}
          />
        ...
        <div className="text-center">
          <div
            className={`w-3 h-3 rounded-full mx-auto mb-1 ${
              integrationStatus?.dataSyncHealth?.notificationDelivery?.status === "healthy"
                ? "bg-green-500"
                : integrationStatus?.dataSyncHealth?.notificationDelivery?.status === "warning"
                  ? "bg-yellow-500"
                  : "bg-red-500"
            }`}
          />
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {check.totalEntities} entities, {Math.round(check.consistencyRatio * 100)}%
                      synced
                    </p>
                  </div>
                  <div
                    className={`w-3 h-3 rounded-full ${
                      check.consistencyRatio >= 0.9
                        ? "bg-green-500"
                        : check.consistencyRatio >= 0.7
                          ? "bg-yellow-500"
                          : "bg-red-500"
                    }`}
                  />
