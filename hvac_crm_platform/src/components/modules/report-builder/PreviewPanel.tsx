    ...
    switch (config.visualization.type) {
          ...
          <ResponsiveContainer width="100%" height={200}>
            <RechartsPieChart>
              <Pie
                ...
                labelLine={false}
              >
                {data.slice(0, 5).map((_entry: any, index: number) => (
      ...
      case "kpi_card": {
        ...
        const formattedValue =
          config.visualization.format === "currency"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center p-2 bg-green-50 rounded">
                ...
                <span className="text-green-800">{previewData.metadata.totalRows} rows</span>
              </div>
              <div className="flex items-center p-2 bg-blue-50 rounded">
                <Clock className="w-3 h-3 text-blue-600 mr-1" />
                <span className="text-blue-800">{executionTime}ms</span>
              </div>
            </div>
              ...
              <div className="p-2 bg-purple-50 rounded">
                ...
                <div className="text-xs text-purple-700 space-y-1">
                  {previewData.metadata.warsawMetrics.affluenceScore && (
                    <div>
                      Affluence: {previewData.metadata.warsawMetrics.affluenceScore.toFixed(2)}x
                    </div>
                  )}
                  {previewData.metadata.warsawMetrics.routeEfficiency && (
                    <div>
                      Route Efficiency:{" "}
                      {previewData.metadata.warsawMetrics.routeEfficiency.toFixed(1)}%
                    </div>
                  )}
                  {previewData.metadata.warsawMetrics.seasonalFactor && (
                    <div>
                      Seasonal Factor:{" "}
                      {previewData.metadata.warsawMetrics.seasonalFactor.toFixed(2)}x
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Visualization */}
            <div className="border border-gray-200 rounded-md p-3">{renderVisualization()}</div>
