import { Calendar, Database, GripVertical, Hash, ToggleLeft, Type } from "lucide-react";
  ...
  return (
    <div
      ...
      className={`flex items-center space-x-2 p-2 rounded cursor-move transition-all ${
        isDragging ? "opacity-50 bg-blue-100 shadow-lg" : "hover:bg-gray-50 hover:shadow-sm"
      }`}
    >
      <GripVertical className="w-3 h-3 text-gray-400" />
      <TypeIcon className={`w-4 h-4 ${getTypeColor()}`} />
      <div className="flex-1 min-w-0">
        <span className="text-sm font-medium text-gray-900 truncate block">{field}</span>
