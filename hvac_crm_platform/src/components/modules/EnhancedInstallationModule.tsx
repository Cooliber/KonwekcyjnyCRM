...
const DEFAULT_STATUSES: InstallationStatus[] = [
  { id: 'planned', name: 'Zaplanowany', color: '#3b82f6', order: 1, isActive: true, automaticActions: [] },
  { id: 'confirmed', name: 'Potwierdzony', color: '#10b981', order: 2, isActive: true, automaticActions: [] },
  { id: 'in_progress', name: 'W trakcie', color: '#f59e0b', order: 3, isActive: true, automaticActions: [] },
  { id: 'testing', name: 'Testowanie', color: '#8b5cf6', order: 4, isActive: true, automaticActions: [] },
  { id: 'completed', name: 'Zakończony', color: '#22c55e', order: 5, isActive: true, requiresDate: true, automaticActions: [] },
  { id: 'cancelled', name: 'Anulowany', color: '#ef4444', order: 6, isActive: true, automaticActions: [] },
  { id: 'postponed', name: 'Przełożony', color: '#6b7280', order: 7, isActive: true, automaticActions: [] }
];
...
export function EnhancedInstallationModule() {
  ...
  const [statuses, setStatuses] = useState<InstallationStatus[]>(DEFAULT_STATUSES);
  ...
  const createInstallation = useMutation(api.jobs.create);
  const updateInstallation = useMutation(api.jobs.update);
  ...
  const stats: InstallationStats = React.useMemo(() => {
    ...
    const byPriority = installations.reduce((acc, inst) => {
    ...
    const onTime = completed.filter(inst => 
      inst.actualEndDate && inst.scheduledDate && inst.actualEndDate <= inst.scheduledDate
    );
  ...
  const handleStatusChange = async (installationId: string, newStatusId: string) => {
    try {
      await updateStatus({
        installationId,
  ...
  const getStatusConfig = (statusId: string): InstallationStatus => {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Zarządzanie montażami HVAC • {stats.total} aktywnych
          </p>
        </div>
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        installation.priority === 'urgent' ? 'bg-red-100 text-red-700' :
                        installation.priority === 'high' ? 'bg-orange-100 text-orange-700' :
                        installation.priority === 'normal' ? 'bg-blue-100 text-blue-700' :
