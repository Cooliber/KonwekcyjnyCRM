...
export function FileUploadManager({
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    const validFiles = acceptedFiles.filter(file => {
      if (file.size > FILE_UPLOAD_CONFIG.MAX_FILE_SIZE) {
        toast.error(`${file.name} is too large. Maximum size is ${FILE_UPLOAD_CONFIG.MAX_FILE_SIZE / 1024 / 1024}MB`);
        return false;
      }
      return true;
    });
  ...
  const getFileType = (file: File): FileUpload['type'] => {
    ...
    } else if (fileType === 'application/pdf' || fileName.includes('invoice') || fileName.includes('faktura')) {
      return 'invoice';
    } else if (fileName.includes('drawing') || fileName.includes('blueprint') || fileName.includes('schema')) {
  ...
  const mapTypeToCategory = (type: FileUpload['type']): string => {
    switch (type) {
      ...
      case 'technical_drawing': return 'manual';
          ...
          {isDragActive ? (
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Supports: Images, PDFs, Documents (max {FILE_UPLOAD_CONFIG.MAX_FILE_SIZE / 1024 / 1024}MB each)
              </p>
            </div>
          )}
        </div>
