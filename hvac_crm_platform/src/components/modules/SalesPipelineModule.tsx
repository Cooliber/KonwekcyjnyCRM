...
export function SalesPipelineModule() {
  ...
  const [viewMode, setViewMode] = useState<'kanban' | 'table' | 'forecast' | 'ai-insights'>('kanban');
  ...
  const pipelineStats = useMemo(() => {
    ...
    const conversionRate = filteredDeals.filter(d => d.stage === 'closed_won').length /
                          filteredDeals.length * 100 || 0;

    // AI-enhanced metrics
    const avgAIScore = aiEnabled ?
      filteredDeals.reduce((sum, deal) => sum + (deal.aiScore || 0), 0) / filteredDeals.length || 0 : 0;

    const highPotentialDeals = filteredDeals.filter(deal => (deal.aiScore || 0) > 70).length;
    const atRiskDeals = filteredDeals.filter(deal =>
      deal.aiInsights?.riskFactors && deal.aiInsights.riskFactors.length > 0
    ).length;
    ...
    const districtPerformance = warsawOptimization ?
      Object.keys(WARSAW_DISTRICT_DATA).map(district => {
        ...
        const districtConversion = districtDeals.filter(d => d.stage === 'closed_won').length /
                                  districtDeals.length * 100 || 0;
        ...
        return {
          ...
          avgAIScore: districtDeals.reduce((sum, deal) => sum + (deal.aiScore || 0), 0) / districtDeals.length || 0
        };
  ...
  const renderKanbanView = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {DEFAULT_STAGES.map(stage => {
        ...
        const avgAIScore = aiEnabled ?
          stageDeals.reduce((sum, deal) => sum + (deal.aiScore || 0), 0) / stageDeals.length || 0 : 0;
                  ...
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      {aiEnabled && deal.aiScore && (
                        <Badge
                          variant={deal.aiScore > 70 ? "default" : deal.aiScore > 50 ? "secondary" : "outline"}
                          className="text-xs"
                        >
                          <Brain className="w-3 h-3 mr-1" />
                          {deal.aiScore}
              ...
              {(['kanban', 'forecast', 'ai-insights'] as const).map(mode => (
                ...
                >
                  {mode === 'kanban' ? 'Pipeline' :
                   mode === 'forecast' ? 'Forecast' : 'AI Insights'}
                </button>
              ))}
            </div>
          </div>
