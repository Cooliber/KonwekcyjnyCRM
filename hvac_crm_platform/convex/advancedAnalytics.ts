...
export const getRevenueMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const contractValue = contracts.reduce((sum, contract) => sum + contract.totalValue, 0);
    ...
    const previousInvoices = await ctx.db.query("invoices")
      .filter(q => 
        q.and(
...
export const getCustomerMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const recentJobs = await ctx.db.query("jobs")
      .filter(q => q.gte(q.field("_creationTime"), startDate))
      .collect();

    const activeCustomerIds = new Set(recentJobs.map(job => job.contactId));
    const retentionRate = customers.length > 0 ? (activeCustomerIds.size / customers.length) * 100 : 0;

    // Calculate satisfaction from service agreements
    let serviceAgreements;
    if (args.district) {
      serviceAgreements = await ctx.db.query("serviceAgreements")
    ...
    const avgSatisfaction = serviceAgreements.length > 0 
      ? serviceAgreements.reduce((sum, sa) => sum + sa.satisfactionScore, 0) / serviceAgreements.length 
      : 0;
...
export const getServiceMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const efficiency = jobsWithTime.length > 0
    ...
    const avgResponseTime = jobsWithSchedule.length > 0
...
export const getEquipmentMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const avgEfficiency = equipment.length > 0
      ? equipment.reduce((sum, eq) => sum + (eq.performance?.efficiency || 85), 0) / equipment.length
      : 87.5;
...
async function calculateAnalytics(ctx: any, startDate: number, district?: string) {
