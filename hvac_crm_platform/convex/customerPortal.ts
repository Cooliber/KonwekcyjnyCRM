...
export const getCustomerPortalUsers = query({
  ...
  handler: async (ctx, args) => {
    ...
    let dbQuery = ctx.db.query("customerPortalUsers");
    ...
    if (args.role) {
      dbQuery = dbQuery.filter((q) => q.eq(q.field("role"), args.role));
    }

    let users = await dbQuery.collect();
...
export const getCustomerPortalUserById = query({
  ...
  handler: async (ctx, args) => {
    ...
    let serviceHistory = await ctx.db
      .query("jobs")
...
export const getCustomerDashboardData = query({
  ...
  handler: async (ctx, args) => {
    ...
    const serviceAgreements = await ctx.db
      .query("serviceAgreements")
      .filter((q) =>
        q.and(q.eq(q.field("clientId"), args.contactId), q.eq(q.field("status"), "active"))
      )
      .collect();

    // Get recent jobs
    let recentJobs = await ctx.db
      .query("jobs")
    ...
    const pendingInvoices = await ctx.db
      .query("invoices")
      .filter((q) =>
        q.and(q.eq(q.field("contactId"), args.contactId), q.neq(q.field("status"), "paid"))
      )
      .collect();
    ...
    const nextServiceDate = serviceAgreements.reduce(
      (earliest, sa) => {
    ...
    const jobId = await ctx.db.insert("jobs", {
      ...
      priority:
        args.urgency === "emergency"
          ? "urgent"
          : (args.urgency as "low" | "medium" | "high" | "urgent"),
      contactId: args.contactId,
      scheduledDate: args.preferredDate,
      estimatedHours: 2, // Default estimate
      equipmentUsed: args.equipmentIds
        ? args.equipmentIds.map((id) => ({ equipmentId: id, quantity: 1 }))
        : undefined,
