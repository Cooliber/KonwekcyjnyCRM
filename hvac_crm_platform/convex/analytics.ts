...
export const getPerformanceMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const responseTimesByService = integrationLogs.reduce((acc, log) => {
      if (!acc[log.service]) {
        acc[log.service] = [];
      }
      // Mock response time calculation (in real implementation would be tracked)
      const responseTime = log.status === "success" ? 
    ...
    const errorRatesByService = integrationLogs.reduce((acc, log) => {
      if (!acc[log.service]) {
...
export const getRouteAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const routeEfficiencyByDistrict = filteredRoutes.reduce((acc, route) => {
    ...
    const overallEfficiency = filteredRoutes.length > 0 ?
      filteredRoutes.reduce((sum, route) => sum + (route.efficiency || 0.5), 0) / filteredRoutes.length :
      0;
    ...
    return {
      ...
      averagePointsPerRoute: filteredRoutes.length > 0 ?
        Math.round(filteredRoutes.reduce((sum, route) => sum + route.points.length, 0) / filteredRoutes.length) :
        0
    };
  },
});
...
export const getProphecyAccuracy = query({
  ...
  handler: async (ctx, args) => {
    ...
    const overallAccuracy = totalPredictions > 0 ? 
      (successfulPredictions / totalPredictions) * 100 : 0;
...
export const getBusinessIntelligence = query({
  ...
  handler: async (ctx, args) => {
    ...
    const jobCompletionRate = jobs.length > 0 ? 
      (jobs.filter(job => job.status === 'completed').length / jobs.length) * 100 : 0;

    // Customer acquisition metrics
    const newCustomers = contacts.filter(contact => 
      contact._creationTime >= args.dateFrom && contact._creationTime <= args.dateTo
    ).length;

    // District performance analysis
    const districtPerformance = invoices.reduce((acc, invoice) => {
...
export const getDashboardMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const jobTrend = weeklyJobs.length > 0 ? 
      ((recentJobs.length * 7) / weeklyJobs.length - 1) * 100 : 0;
