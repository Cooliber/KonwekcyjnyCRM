...
const DISTRICT_AFFLUENCE_MULTIPLIERS = {
  ...
  Wilanów: 1.2, // High affluence - 20% premium
  Mokotów: 1.15, // High affluence - 15% premium
  Żoliborz: 1.1, // Medium-high affluence - 10% premium
  Ursynów: 1.05, // Medium affluence - 5% premium
  Wola: 1.0, // Base pricing
  "Praga-Południe": 0.95, // Lower affluence - 5% discount
  Targówek: 0.9, // Lowest affluence - 10% discount
};

// Service type pricing multipliers
const SERVICE_TYPE_MULTIPLIERS = {
  emergency: 1.5, // 50% emergency surcharge
  installation: 1.0, // Base pricing
  maintenance: 0.85, // 15% discount for maintenance
  repair: 1.1, // 10% premium for repairs
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    let dbQuery = ctx.db.query("invoices");
    ...
    if (args.contactId) {
      dbQuery = dbQuery.filter((q) => q.eq(q.field("contactId"), args.contactId));
    }
    ...
    const enhancedInvoices = await Promise.all(
      invoices.map(async (invoice) => {
        ...
        return {
          ...
          contact,
          job,
          districtMultiplier: contact?.district
            ? DISTRICT_AFFLUENCE_MULTIPLIERS[
                contact.district as keyof typeof DISTRICT_AFFLUENCE_MULTIPLIERS
              ] || 1.0
            : 1.0,
        };
      })
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.applyServiceTypePricing && job.type) {
      pricingMultiplier *=
        SERVICE_TYPE_MULTIPLIERS[job.type as keyof typeof SERVICE_TYPE_MULTIPLIERS] || 1.0;
    }
    ...
    // Calculate totals
    const subtotal = adjustedItems.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);

    const totalTax = adjustedItems.reduce(
...
export const getRevenueAnalytics = query({
  args: {
    ...
    dateTo: v.number(),
    groupBy: v.optional(
      v.union(v.literal("district"), v.literal("month"), v.literal("service_type"))
    ),
  },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    ...
    const analytics: any = {
      ...
      averageInvoiceValue:
        invoices.length > 0
          ...
          : 0,
      totalEfficiencyDiscounts: invoices.reduce(
        (sum, inv) => sum + (inv.efficiencyDiscount || 0),
        0
      ),
    };
      ...
      enhancedInvoices.forEach(({ invoice, contact }) => {
        ...
        if (!analytics.byDistrict[district]) {
          analytics.byDistrict[district] = {
            ...
            multiplier:
              DISTRICT_AFFLUENCE_MULTIPLIERS[
                district as keyof typeof DISTRICT_AFFLUENCE_MULTIPLIERS
              ] || 1.0,
          };
        }
      ...
      enhancedInvoices.forEach(({ invoice, job }) => {
        ...
        if (!analytics.byServiceType[serviceType]) {
          analytics.byServiceType[serviceType] = {
            ...
            averageValue: 0,
            multiplier:
              SERVICE_TYPE_MULTIPLIERS[serviceType as keyof typeof SERVICE_TYPE_MULTIPLIERS] || 1.0,
          };
        }
...
async function generateInvoiceNumber(ctx: any): Promise<string> {
  ...
  const monthlyInvoices = await ctx.db
    .query("invoices")
    .filter((q: any) =>
      q.and(q.gte(q.field("issueDate"), monthStart), q.lte(q.field("issueDate"), monthEnd))
    )
    .collect();
...
async function calculateBaselineRevenue(
  ctx: any,
  dateFrom: number,
  dateTo: number
): Promise<number> {
