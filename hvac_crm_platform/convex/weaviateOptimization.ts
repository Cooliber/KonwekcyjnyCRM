...
const WEAVIATE_CONFIG = {
  ...
  // Cache keys
  VECTOR_SEARCH_KEY: (query: string, filters: any) => `vector_search_${query}_${JSON.stringify(filters)}`,
...
export const batchVectorOperations = action({
  ...
  handler: async (ctx, args): Promise<BatchSearchResult> => {
    ...
    if (embedOps.length > 0) {
      embeddingCount = embedOps.length;
...
function applyDistrictWeighting(results: any[], district?: string): VectorSearchResponse {
  ...
  })).sort((a, b) => (b._additional?.weightedCertainty || 0) - (a._additional?.weightedCertainty || 0));

  return {
    results: weightedResults.map(r => ({ id: r.id || 'unknown', score: r._additional?.weightedCertainty || 0, data: r })),
...
function buildWeaviateFilter(type: string, district?: string, filters?: any): any {
  const conditions = [];
...
async function batchUpdateWeaviate(operations: any[]): Promise<any[]> {
