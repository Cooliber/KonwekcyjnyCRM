...
type DistrictName = 'Śródmieście' | 'Wilanów' | 'Mokotów' | 'Żoliborz' | 'Ursynów' | 'Wola' | 'Praga-Południe' | 'Targówek';
...
export const getOptimizedNotifications = query({
  ...
  handler: async (ctx, args) => {
    ...
    const ttl = args.priority === 'urgent' 
...
export const batchCreateNotifications = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return { created: results.length, batchCount: Math.ceil(args.notifications.length / batchSize) };
  },
});
...
function getOperationLimit(operation: string): number {
  switch (operation) {
    case 'messages': return PERFORMANCE_CONFIG.RATE_LIMITS.MESSAGES_PER_MINUTE;
    case 'notifications': return PERFORMANCE_CONFIG.RATE_LIMITS.NOTIFICATIONS_PER_MINUTE;
    case 'batch_notifications': return 10; // Lower limit for batch operations
    default: return PERFORMANCE_CONFIG.RATE_LIMITS.API_CALLS_PER_MINUTE;
  }
}
...
function recordPerformanceMetric(type: 'cache_hit' | 'cache_miss', responseTime: number): void {
  performanceMetrics.totalRequests++;
